...
function getBackUrls(orderId) {
  const front = resolveFrontBase();
  const base = front.replace(/\/$/, '');
  const qp = orderId ? `?order_id=${encodeURIComponent(orderId)}` : '';
  return {
    success: `${base}/postpago.html${qp}`,
    failure: `${base}/postpago.html${qp}`,
    pending: `${base}/postpago.html${qp}`,
  };
}
...
async function fetchMpPayment(paymentId) {
  // Prefer SDK, but fall back to direct HTTP in case of SDK mismatch.
  try {
    const r = await mpPayment.get({ id: String(paymentId) });
    if (r?.response) return r;
  } catch (e) {
    // ignore and fall back
  }

  const token = process.env.MP_ACCESS_TOKEN || process.env.ACCESS_TOKEN || '';
  ...
  if (!resp.ok) {
    const err = new Error(`MP payment fetch failed (${resp.status})`);
    err.details = data;
    throw err;
  }
...
app.get('/api/debug/mp', async (_req, res) => {
  try {
    const back_urls = getBackUrls(orderId);
...
async function processPaymentAndMaybeApprove({ orderId, paymentId, paymentStatus, payerEmail, items }) {
  ...
  try {
    ...
    // Intento 1: actualizar columnas completas (si existen)
    try {
      await client.query(
        `UPDATE orders
           SET status = $1,
      ...
      // Si faltan columnas (42703), intentamos una versión más simple
      if (err?.code === '42703') {
        try {
          await client.query(
            `UPDATE orders
               SET status = $1,
                   payment_id = $2
             WHERE order_id = $3`,
            [newStatus, paymentId || prevPaymentId, orderId]
          );
        } catch (err2) {
          if (err2?.code === '42703') {
            await client.query('UPDATE orders SET status = $1 WHERE order_id = $2', [newStatus, orderId]);
          } else {
            throw err2;
          }
        }
    ...
    // Activar domicilio SOLO si el pago quedó aprobado (si existen esas columnas)
    if (
      newStatus === 'approved' &&
      order.domicilio_modo &&
      ('estado_domicilio' in order) &&
      !order.estado_domicilio
    ) {
      try {
        await client.query(
          `UPDATE orders
...
app.post('/api/payments/confirm', async (req, res) => {
  try {
    ...
    if (!p) return res.status(400).json({
      ...
      hint: 'No pude consultar el pago en Mercado Pago. Revisa MP_ACCESS_TOKEN/MP_PUBLIC_KEY (test vs prod) y usa un payment_id reciente que pertenezca a esa cuenta.',
    });

    const orderId = Number(p.external_reference || p.metadata?.order_id);
    if (!orderId) return res.status(400).json({
      error: 'missing_order_reference',
      hint: 'El pago no trae external_reference/metadata.order_id. Confirma que /api/payments/create envía external_reference y metadata.order_id.',
    });
    ...
    return res.status(500).json({
      ...
      hint: 'Si ves payment_not_found, casi siempre es credencial equivocada (TEST vs APP_USR) o MP_ACCESS_TOKEN faltante en Cloud Run.',
    });
  }
});
...
app.get('/api/invoices/:orderId', async (req, res) => {
  try {
    ...
    // 1) Pedido (SELECT * para no romper si faltan columnas en la BD)
    const oRes = await pool.query('SELECT * FROM orders WHERE order_id = $1', [orderId]);
    if (!oRes.rows.length) return res.status(404).json({ error: 'order_not_found' });
    const order = oRes.rows[0];

    // Defaults/aliases para el front
    order.customer = order.customer || order.customer_name || 'Cliente';
    order.email = order.email || order.payer_email || order.customer_email || '';
    order.domicilio_pais = order.domicilio_pais || 'Colombia';
    order.domicilio_costo = Number(order.domicilio_costo || 0);
